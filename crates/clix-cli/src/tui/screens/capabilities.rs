    ...
    if let Some(ns) = namespaces.get(app.caps_cursor) {
        ...
        let mut lines = vec![
            ...
            Line::from(Span::styled(
                format!("  {} — {} capabilities", ns.key, ns.count),
                theme::accent(),
            )),
            Line::from(""),
        ];
    ...
    if let Some(cap) = caps.get(app.caps_cursor) {
        ...
        let lines = vec![
            ...
            Line::from(vec![
                ...
                Span::styled(cap.name.as_str(), theme::normal()),
            ]),
            Line::from(vec![
                Span::styled("  Backend    ", theme::muted()),
                Span::styled(backend_str, theme::dim()),
            ]),
            Line::from(vec![
            ...
            Line::from(vec![
                ...
                Span::styled(
                    format!("  {}", se_label(&cap.side_effect_class)),
                    Style::default().fg(se_col),
                ),
            ]),
            Line::from(vec![
                Span::styled("  Desc       ", theme::muted()),
                Span::styled(cap.description.as_deref().unwrap_or("—"), theme::dim()),
            ]),
            Line::from(""),
            Line::from(vec![
...
fn render_detail(f: &mut Frame, app: &App, area: Rect, name: &str) {
    ...
    if let Some(cap) = app.registry.get(name) {
        ...
        let schema_str =
            serde_json::to_string_pretty(&cap.input_schema).unwrap_or_else(|_| "(none)".into());

        let mut lines = vec![
            Line::from(""),
            Line::from(vec![
                Span::styled("  Name         ", theme::muted()),
                Span::styled(cap.name.as_str(), theme::accent()),
            ]),
            Line::from(vec![
            ...
            Line::from(vec![
                Span::styled("  Backend      ", theme::muted()),
                Span::styled(backend_str, theme::normal()),
            ]),
            Line::from(vec![
                Span::styled("  Risk         ", theme::muted()),
                Span::styled(risk_label(&cap.risk), Style::default().fg(risk_col)),
