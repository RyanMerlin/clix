use super::infisical_setup::{InfisicalSetupAction, InfisicalSetupState};
use crate::tui::theme;
...
impl InfisicalAccountsState {
            ...
            _ => {
                ...
                match action {
                    ...
                    InfisicalSetupAction::Save {
                        ...
                        if name.is_empty() {
                            self.status =
                                Some(("Enter a profile name first (press BackTab)".into(), true));
                        } else if self.profiles.iter().any(|(n, _)| n == &name) {
                            self.status =
                                Some((format!("Profile '{}' already exists", name), true));
                        } else {
                            self.mode = AccountsMode::List;
                            return AccountsAction::Save {
                                name,
                                site_url,
                                client_id,
                                client_secret,
    ...
    pub fn render(&self, f: &mut Frame, area: Rect) {
        let width = area.width.saturating_sub(4).clamp(60, 80);
    ...
    fn render_list(&self, f: &mut Frame, area: Rect) {
        ...
        let status_line = if let Some((ref msg, is_err)) = self.status {
            Span::styled(
                msg.as_str(),
                if is_err { theme::danger() } else { theme::ok() },
            )
        } else {
            Span::styled(
                "a:add  e/enter:edit  u:use  d:remove  t:test  esc:close",
                theme::muted(),
            )
        };
