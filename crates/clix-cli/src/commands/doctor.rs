...
pub fn run(json: bool) -> Result<()> {
    ...
    let receipts_summary =
        format!("{r_total} total  ({r_allowed} allowed · {r_denied} denied · {r_failed} failed)");
...
fn check_worker() -> (&'static str, String) {
    ...
    if !binary.exists() {
        return (
            "✗ worker",
            format!(
                "clix-worker not found (expected alongside clix at {})",
                binary.display()
            ),
        );
    }
