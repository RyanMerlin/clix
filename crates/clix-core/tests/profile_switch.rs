use clix_core::execution::worker_registry::{WorkerKey, WorkerRegistry};
...
fn test_profile_policy_isolation_no_cross_contamination() {
    ...
    let infisical = clix_core::state::InfisicalProfiles {
    ...
    assert_eq!(
        ro_receipts.len(),
        1,
        "readonly store should have 1 denied receipt"
    );
    assert_eq!(
        rw_receipts.len(),
        1,
        "write store should have 1 succeeded receipt"
    );
}
