...
fn rule_matches(rule: &PolicyRule, ctx: &ExecutionContext, cap: &CapabilityManifest) -> bool {
    ...
    if let Some(ref risk) = rule.risk
        && risk != &risk_label(&cap.risk)
    {
        return false;
    }
    if let Some(ref sec) = rule.side_effect_class
        && std::mem::discriminant(sec) != std::mem::discriminant(&cap.side_effect_class)
    {
        return false;
    }
    true
...
mod tests {
    ...
    fn unmatched_capability_is_denied_by_default() {
        ...
        assert!(matches!(
            evaluate_policy(&policy, &ctx(), &stub_cap("sys.date", RiskLevel::Low)),
            Decision::Deny { .. }
        ));
    }
    ...
    fn unmatched_low_risk_allows_when_default_allow() {
        ...
        assert!(matches!(
            evaluate_policy(&allow_all(), &ctx(), &stub_cap("sys.date", RiskLevel::Low)),
            Decision::Allow
        ));
    }
    ...
    fn unmatched_high_risk_requires_approval_when_default_allow() {
        assert!(matches!(
            evaluate_policy(
                &allow_all(),
                &ctx(),
                &stub_cap("k8s.apply", RiskLevel::High)
            ),
    ...
    fn test_deny_by_name() {
        ...
        policy.rules.push(PolicyRule {
            capability: Some("bad.cmd".to_string()),
            action: PolicyAction::Deny,
            reason: Some("no".to_string()),
            ..Default::default()
        });
        assert!(matches!(
            evaluate_policy(&policy, &ctx(), &stub_cap("bad.cmd", RiskLevel::High)),
            Decision::Deny { .. }
        ));
    }
