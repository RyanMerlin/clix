...
pub fn apply_sandbox(allowed_executables: &[String]) -> Result<()> {
    ...
    let mut ruleset = Ruleset::default()
        ...
        .map_err(|e| ClixError::Sandbox(format!("landlock: {e}")))?
        .create()
        .map_err(|e| ClixError::Sandbox(format!("landlock create: {e}")))?;
    for path in allowed_executables {
        let fd = PathFd::new(path).map_err(|e| ClixError::Sandbox(format!("landlock path {path}: {e}")))?;
        ruleset = ruleset.add_rule(PathBeneath::new(fd, AccessFs::Execute))
            .map_err(|e| ClixError::Sandbox(format!("landlock rule: {e}")))?;
    }
    ruleset.restrict_self().map_err(|e| ClixError::Sandbox(format!("landlock restrict: {e}")))?;
    Ok(())
}
