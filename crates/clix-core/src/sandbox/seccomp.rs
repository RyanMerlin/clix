...
const BASELINE_DENY: &[&str] = &[
    ...
    "setfsgid",
    "capset",
    "prctl",         // blocked except PR_SET_NAME; see note below
...
pub fn build_filter(extra_deny: &[&str]) -> Result<SeccompFilter> {
    ...
    insns.push(bpf_stmt(BPF_RET | BPF_K, 0x0000_0000u32)); // SECCOMP_RET_KILL_PROCESS
...
pub fn install_filter(filter: &SeccompFilter) -> Result<()> {
    ...
    if ret != 0 {
        return Err(ClixError::Sandbox(format!(
...
fn syscall_nr(name: &str) -> Option<i64> {
    ...
    let table: &[(&str, i64)] = &[
        ...
        ("set_mempolicy_home_node", 450), ("stime", 160),
    ];
