...
///      c. Sets up the mount namespace: tmpfs root, RO bind of pinned binary + ldd libs,
///         proc, tmpfs /tmp, tmpfs /home/clix.
...
use crate::manifest::capability::{FsPolicy, NetworkPolicy, CgroupLimits};
...
pub mod env_keys {
    pub const PINNED_BINARY: &str = "CLIX_JAIL_BINARY";
    pub const BINARY_SHA256: &str = "CLIX_JAIL_SHA256";
    pub const LIB_PATHS: &str = "CLIX_JAIL_LIBS";           // colon-separated
    pub const EXTRA_RO_BIND: &str = "CLIX_JAIL_RO_BIND";    // colon-separated
    pub const EXTRA_RW_BIND: &str = "CLIX_JAIL_RW_BIND";    // colon-separated
    pub const EXTRA_DENY_SYSCALLS: &str = "CLIX_JAIL_DENY_SYSCALLS"; // comma-separated
    pub const MEMORY_MIB: &str = "CLIX_JAIL_MEM_MIB";
    pub const MAX_PIDS: &str = "CLIX_JAIL_MAX_PIDS";
    pub const WORKER_SOCKET_FD: &str = "CLIX_WORKER_FD";
    pub const SHARE_HOST_TMP: &str = "CLIX_JAIL_SHARE_TMP";
}
...
impl JailConfig {
    ...
    pub fn from_env() -> Option<Self> {
        ...
        Some(JailConfig {
            ...
            limits: CgroupLimits { memory_mib, max_pids, cpu_weight: 100 },
...
pub fn resolve_and_hash_binary(command: &str) -> Result<(PathBuf, String)> {
    ...
    let bytes = fs::read(&path)
        .map_err(|e| ClixError::Isolation(format!("read binary {}: {e}", path.display())))?;
...
pub fn verify_binary_hash(path: &Path, expected_sha256: &str) -> Result<()> {
    ...
    if actual != expected_sha256 {
        return Err(ClixError::IntegrityFailure(format!(
...
pub fn discover_lib_deps(binary: &Path) -> Vec<PathBuf> {
    ...
    for line in stdout.lines() {
        ...
        let path_str = if let Some(pos) = line.find("=>") {
            // "name => /abs/path (0x...)"
            line[pos + 2..].trim().split_whitespace().next().unwrap_or("")
        } else {
            // "/abs/path (0x...)"
...
pub fn enter_jail(config: &JailConfig) -> Result<()> {
    ...
    if unsafe { libc::pipe(pipe_a.as_mut_ptr()) } != 0 {
        return Err(ClixError::Isolation(format!("pipe_a: {}", std::io::Error::last_os_error())));
    }
    if unsafe { libc::pipe(pipe_b.as_mut_ptr()) } != 0 {
        return Err(ClixError::Isolation(format!("pipe_b: {}", std::io::Error::last_os_error())));
    }
    ...
    if child_pid < 0 {
        return Err(ClixError::Isolation(format!("fork: {}", std::io::Error::last_os_error())));
    }
    ...
    if child_pid == 0 {
        ...
        if let Err(e) = unshare_namespaces() {
            eprintln!("[clix-jail] unshare failed: {e}");
        ...
        if let Err(e) = setup_mount_namespace(&root_path, config) {
            eprintln!("[clix-jail] mount namespace failed: {e}");
            unsafe { libc::_exit(127) };
        }

        // Step 6: pivot_root
        if let Err(e) = pivot_into(&root_path) {
            eprintln!("[clix-jail] pivot_root failed: {e}");
            unsafe { libc::_exit(127) };
        }

        // Step 7: NO_NEW_PRIVS
        if let Err(e) = set_no_new_privs() {
            eprintln!("[clix-jail] no_new_privs failed: {e}");
            unsafe { libc::_exit(127) };
        }
...
fn unshare_namespaces() -> Result<()> {
    ...
    if ret != 0 {
        return Err(ClixError::Isolation(format!(
...
fn setup_mount_namespace(root: &Path, config: &JailConfig) -> Result<()> {
    ...
    let root_c = CString::new(root.to_string_lossy().as_bytes())
        .map_err(|e| ClixError::Isolation(format!("root cstring: {e}")))?;
    ...
    for dir in &["proc", "tmp", "home", "home/clix", "usr", "lib", "lib64", "bin", "dev"] {
    ...
    let bin_name = config.pinned_binary.file_name()
        .ok_or_else(|| ClixError::Isolation("binary has no filename".to_string()))?;
    let jail_bin = root.join("bin").join(bin_name);
    fs::write(&jail_bin, "").map_err(|e| ClixError::Isolation(format!("create bind target: {e}")))?;
    ...
    for lib in &config.lib_paths {
        if lib.is_dir() {
            ...
            fs::create_dir_all(&dest)
                .map_err(|e| ClixError::Isolation(format!("mkdir lib dir: {e}")))?;
            ...
            if let Some(parent) = dest.parent() {
                fs::create_dir_all(parent)
                    .map_err(|e| ClixError::Isolation(format!("mkdir lib parent: {e}")))?;
            }
            fs::write(&dest, "").map_err(|e| ClixError::Isolation(format!("create lib target: {e}")))?;
            bind_mount(lib, &dest, true)?;
        }
    }
    ...
    for path_str in &config.fs_policy.extra_ro_bind {
        ...
        if src.exists() {
            ...
            if src.is_dir() {
                fs::create_dir_all(&dest)
                    .map_err(|e| ClixError::Isolation(format!("mkdir extra ro: {e}")))?;
            } else {
                if let Some(p) = dest.parent() {
                    fs::create_dir_all(p)
                        .map_err(|e| ClixError::Isolation(format!("mkdir extra ro parent: {e}")))?;
                }
                fs::write(&dest, "").ok();
            }
    ...
    for path_str in &config.fs_policy.extra_rw_bind {
        ...
        if src.exists() {
            ...
            if src.is_dir() {
                fs::create_dir_all(&dest)
                    .map_err(|e| ClixError::Isolation(format!("mkdir extra rw: {e}")))?;
            } else {
                if let Some(p) = dest.parent() {
                    fs::create_dir_all(p)
                        .map_err(|e| ClixError::Isolation(format!("mkdir extra rw parent: {e}")))?;
                }
                fs::write(&dest, "").ok();
            }
...
fn bind_mount(src: &Path, dest: &Path, read_only: bool) -> Result<()> {
    ...
    let src_c = CString::new(src.to_string_lossy().as_bytes())
        .map_err(|e| ClixError::Isolation(format!("bind src cstring: {e}")))?;
    let dest_c = CString::new(dest.to_string_lossy().as_bytes())
        .map_err(|e| ClixError::Isolation(format!("bind dest cstring: {e}")))?;
...
fn pivot_into(new_root: &Path) -> Result<()> {
    ...
    fs::create_dir_all(&put_old)
        .map_err(|e| ClixError::Isolation(format!("mkdir old_root: {e}")))?;
    ...
    std::env::set_current_dir("/")
        .map_err(|e| ClixError::Isolation(format!("chdir to new root: {e}")))?;
...
fn drop_capabilities() -> Result<()> {
    ...
    let ret = unsafe {
        libc::syscall(SYS_CAPSET, &hdr as *const CapHeader, data.as_ptr() as *const CapData)
    };
    if ret != 0 {
        // Non-fatal — NO_NEW_PRIVS + bounding-set drop already denies escalation
        eprintln!("[clix-worker] capset warning: {}", std::io::Error::last_os_error());
    }
    Ok(())
}
...
fn which_binary(command: &str) -> Result<PathBuf> {
    ...
    if p.is_absolute() {
        ...
        return Err(ClixError::Isolation(format!("binary not found: {command}")));
    }
    // Search PATH
    ...
    Err(ClixError::Isolation(format!("binary `{command}` not found on PATH")))
}
...
mod tests {
    ...
    fn test_resolve_nonexistent() {
        assert!(resolve_and_hash_binary("this_binary_does_not_exist_clix").is_err());
    }
