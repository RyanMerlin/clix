...
fn run_remote_http(
    ...
    let resp: serde_json::Value = client
        ...
        .json()
        .map_err(|e| ClixError::Backend(format!("remote decode: {e}")))?;
...
fn run_remote_unix(
    ...
    let mut stream = UnixStream::connect(path)
        .map_err(|e| ClixError::Backend(format!("unix socket {path}: {e}")))?;
    let payload = serde_json::json!({"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":capability_name,"arguments":input}});
    let mut line = serde_json::to_string(&payload).unwrap();
    line.push('\n');
    stream
        .write_all(line.as_bytes())
        .map_err(|e| ClixError::Backend(format!("unix write: {e}")))?;
