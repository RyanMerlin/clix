...
mod tests {
    ...
    fn test_builtin_date() {
        assert!(
            builtin_handler("date", &serde_json::json!({})).unwrap()["date"]
                .as_str()
                .is_some()
        );
    }
    ...
    fn test_builtin_echo() {
        assert_eq!(
            builtin_handler("echo", &serde_json::json!({"message":"hi"})).unwrap()["output"],
            "hi"
        );
    }
    #[test]
    fn test_builtin_unknown() {
        assert!(builtin_handler("nope", &serde_json::json!({})).is_err());
    }
}
