            ...
            ValidatorKind::DenyArgs => {
                ...
                for forbidden in &v.values {
                    if args_str.contains(forbidden.as_str()) {
                        errors.push(format!("Forbidden argument: {forbidden}"));
                    }
                }
            }
...
mod tests {
    ...
    fn test_deny_args() {
        let v = vec![Validator {
            ...
            path: String::new(),
            key: String::new(),
            values: vec!["--force".to_string()],
        }];
        let errs = run_validators(
            &v,
            &serde_json::json!({}),
            std::path::Path::new("."),
            &["kubectl".to_string(), "--force".to_string()],
        );
    ...
    fn test_required_input_key() {
        ...
        assert!(
            !run_validators(&v, &serde_json::json!({}), std::path::Path::new("."), &[]).is_empty()
        );
        assert!(
            run_validators(
                &v,
