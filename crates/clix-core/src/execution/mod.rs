...
pub fn run_capability(
    ...
    profile_secret_bindings: &[ProfileSecretBinding],
    profile_folder_bindings: &[ProfileFolderBinding],
    ...
    match &decision {
        ...
        Decision::RequireApproval { reason } => {
            ...
            if worker_registry.is_some() {
                ...
                match approval::wait_for_broker_approval(
                    receipt_id, &cap.name, &input, &ctx_value, reason,
                ) {
    ...
    let secrets = resolve_credentials(
        &cap.credentials,
        infisical,
        ...
        Backend::Subprocess {
            ...
            let (exit_code, stdout, stderr, isolation_tier) = if let Some(reg) = worker_registry {
                ...
                (
                    ...
                    dispatch.stdout,
                    dispatch.stderr,
                    dispatch.isolation_tier,
                )
            } else {
                #[cfg(target_os = "macos")]
                let sub = backends::run_subprocess_sandboxed(
                    command,
                    &expanded,
                    &cwd,
                    &secrets,
                ...
                (
                    ...
                    sub.stdout,
                    sub.stderr,
                    crate::manifest::capability::IsolationTier::None,
                )
            };
    ...
    store.write(&Receipt {
        ...
        approval: None,
        sandbox_enforced: sandbox_enforced(),
        isolation_tier: isolation_tier_str,
        binary_sha256,
        token_mint_id,
        jail_config_digest: None,
    })?;
...
mod tests {
    ...
    fn no_infisical(
        map: &BTreeMap<String, crate::state::InfisicalConfig>,
    ) -> InfisicalProfiles<'_> {
        InfisicalProfiles {
            profiles: map,
            active: None,
