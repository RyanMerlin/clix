...
pub fn diff_pack(installed: &Path, new_src: &Path) -> Result<DiffReport> {
    ...
    let old_caps: std::collections::HashSet<_> =
        old.capabilities.iter().map(|c| c.name.clone()).collect();
    let new_caps: std::collections::HashSet<_> =
        new.capabilities.iter().map(|c| c.name.clone()).collect();

    let old_profiles: std::collections::HashSet<_> =
        old.profiles.iter().map(|p| p.name.clone()).collect();
    let new_profiles: std::collections::HashSet<_> =
        new.profiles.iter().map(|p| p.name.clone()).collect();

    let old_wf: std::collections::HashSet<_> =
        old.workflows.iter().map(|w| w.name.clone()).collect();
    let new_wf: std::collections::HashSet<_> =
        new.workflows.iter().map(|w| w.name.clone()).collect();
    ...
    let changed: Vec<String> = old
        ...
        .filter_map(|old_cap| {
            new.capabilities
                .iter()
                .find(|nc| nc.name == old_cap.name && nc.version != old_cap.version)
    ...
    Ok(DiffReport {
        ...
        version_change: if old.pack.version != new.pack.version {
            Some((old.pack.version.clone(), new.pack.version.clone()))
        } else {
            None
        },
