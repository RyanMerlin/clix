...
async fn test_deny_by_name() {
    ...
    let serve = make_state(vec![builtin("gcloud.projects.list")], policy);
    let resp = call(&serve, "gcloud.projects.list").await;
    assert!(
        resp["result"]["isError"].as_bool().unwrap_or(false),
        "expected denied: {resp}"
    );
}
...
async fn test_deny_by_side_effect() {
    ...
    assert!(
        !ro_resp["result"]["isError"].as_bool().unwrap_or(true),
        "ReadOnly should succeed"
    );
    let mt_resp = call(&serve, "k8s.pods.delete").await;
    assert!(
        mt_resp["result"]["isError"].as_bool().unwrap_or(false),
        "Mutating should be denied"
    );
}
...
async fn test_require_approval_blocks(
    ...
    let serve = make_state(vec![with_side_effect(cap_name, side_effect)], policy);
    let resp = call(&serve, cap_name).await;
    assert!(
        resp["result"]["isError"].as_bool().unwrap_or(false),
        "expected blocked: {resp}"
    );
    assert!(
        resp["result"]["_clix"]["approvalRequired"]
            .as_bool()
            .unwrap_or(false),
...
async fn test_unknown_capability_returns_error() {
    ...
    assert!(
        is_error,
        "unknown capability should produce an error: {resp}"
    );
}
