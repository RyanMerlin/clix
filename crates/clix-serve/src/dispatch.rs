...
mod tests {
    ...
    use clix_core::manifest::capability::{
        Backend, CapabilityManifest, RiskLevel, SideEffectClass,
    };
    use clix_core::policy::PolicyBundle;
    use clix_core::receipts::ReceiptStore;
    use clix_core::registry::{CapabilityRegistry, WorkflowRegistry};
    ...
    fn test_state() -> Arc<ServeState> {
        let home = unique_home("clix-test-serve");
        Arc::new(ServeState {
    ...
    fn test_state_with_caps() -> Arc<ServeState> {
        let home = unique_home("clix-test-serve-ns");
    ...
    async fn test_initialize() {
        ...
        assert_eq!(
            resp["result"]["capabilities"]["extensions"]["clix"]["namespaces"],
            true
        );
    }
